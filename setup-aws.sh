...
S3_BUCKET=${S3_BUCKET:-crm-demo-bucket}
DYNAMODB_TABLE=${DYNAMODB_TABLE:-customers}

VPC_ID=${VPC_ID}  # Required if creating EFS
EFS_ID=${EFS_ID}  # Optional - will create if not provided
EFS_SECURITY_GROUP=${EFS_SECURITY_GROUP}  # Required if creating EFS
APP_SUBNET_IDS=${APP_SUBNET_IDS}  # Comma-separated subnet IDs for EFS mount targets

DB_SECRET_NAME=${DB_SECRET_NAME:-rds-credentials}
DB_TYPE=${DB_TYPE:-aurora}  # Options: rds or aurora
DB_INSTANCE_ID=${DB_INSTANCE_ID:-demo-db}
DB_CLUSTER_ID=${DB_CLUSTER_ID:-demo-cluster}
DB_USERNAME=${DB_USERNAME:-dbadmin}
DB_PASSWORD=${DB_PASSWORD:-demoPassword}
DB_SUBNET_GROUP=${DB_SUBNET_GROUP:-demo-db-subnet-group}
DB_SUBNET_IDS=${DB_SUBNET_IDS}  # Comma-separated subnet IDs for DB subnet group
DB_SECURITY_GROUP=${DB_SECURITY_GROUP}  # Must be provided
